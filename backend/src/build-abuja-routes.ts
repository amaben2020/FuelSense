...
import { ABUJA_LOOP_DEFINITIONS } from './lib/abuja-routes';

const KEY = process.env.GOOGLE_MAPS_API_KEY;

async function fetchLoop(points: GeoPoint[]): Promise<GeoPoint[]> {
...
(async () => {
  ...
  for (const [name, def] of Object.entries(ABUJA_LOOP_DEFINITIONS)) {
    const path = await fetchLoop(def.stops);
    out[name] = path;
    console.log(`${name.padEnd(10)} ${def.stops.length} stops → ${path.length} road points`);
  }
