...
import { db, devices, telemetry, deviceFrames, vehicles, eq, and, sql } from '../../shared/db-helpers';
import { detectAnomalies } from '../fuel/anomaly-detector.service';
import { invalidate } from '../../config/redis';
import { getIoValue, serializeIo } from './avl-io.service';
import { decodeScenarioEvent, recordDeviceEvent, resolveClosedAlert } from '../devices/device-event-decoder.service';
import { handleIgnitionForTripStart, closeTripStartAlert } from '../telemetry/trip-notifier.service';
import { handleIdleForRecord } from '../telemetry/idle-detector.service';
import { handleGeofenceForRecord } from '../geofences/geofence-monitor.service';
import {
  patchCodec8eIoParsing,
  patchCodec8eReassembly,
  patchCodec8eRecordParsing,
} from './codec8e-io-patch.service';
import { handleFuelStopForRecord } from '../fuel/fuel-stop-detector.service';
import { handlePowerForRecord } from '../devices/power-monitor.service';
import {
  flushQueuedCommand,
  handleCommandReply,
  handleDoutReading,
  registerCommandLink,
} from '../vehicles/immobilizer.service';
import { DOUT1_AVL_ID } from './teltonika-dout.service';
import { publishLivePoint } from '../telemetry/live-feed.service';
import {
  applyBurnCarry,
  shouldSkipTelemetryRow,
  FloorState,
} from './telemetry-write-floor.service';
import {
  recordFrame,
  tcpDevicesConnected,
  tcpHandshakesTotal,
...
} from '../fuel/virtual-tank.service';
 ...
 * and the modelled burn from skipped hops owed to the next row written.
 * Kept together and copied to Redis so a restart neither writes a redundant
 * row nor drops the millilitres carried at that moment.
 */
interface WriteFloorState {
  lastPersisted: FloorState | null;
  carriedMl: number;
}
const writeFloorByImei = new DetectorState<WriteFloorState>('write-floor', {
  ttlSeconds: 24 * 60 * 60,
});
...
const saveTelemetry = async (device: TeltonikaDevice, record: TeltonikaRecord): Promise<void> => {
  try {
    ...
    // telemetry-write-floor.service.ts for the measurements behind the interval.
    const floor = (await writeFloorByImei.get(device.imei)) ?? { lastPersisted: null, carriedMl: 0 };
    const skipTelemetryRow = shouldSkipTelemetryRow({
      recordedAtMs: recordedAt.getTime(),
      ignitionOn,
      eventId: record.event,
      previous: floor.lastPersisted,
    });

    const carry = applyBurnCarry(skipTelemetryRow, burnMl, floor.carriedMl);
    burnMl = carry.burnMl;
    writeFloorByImei.set(device.imei, { ...floor, carriedMl: carry.carriedMl });
    ...
    if (!skipTelemetryRow) {
      ...
      writeFloorByImei.set(device.imei, {
        lastPersisted: { atMs: recordedAt.getTime(), ignitionOn },
        carriedMl: carry.carriedMl,
      });
    }
