...
export const initDatabase = async (): Promise<void> => {
  ...
  {
    const { VEHICLE_TYPE_PRESETS, DEFAULT_VEHICLE_TYPE } = await import('../../features/fuel/fuel-metrics.service');
  ...
  // `driver-portal.routes.ts` and `telemetry.routes.ts` both read the newest frame for
  ...
  const { backfillDriverReceiptPurchases } = await import('../../features/receipts/driver-receipt-sync.service');
  await backfillDriverReceiptPurchases(db);

  const { syncDemoVehicleDrivers } = await import('../../features/vehicles/sync-vehicle-drivers.service');
