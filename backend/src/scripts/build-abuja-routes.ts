...
// Builds src/features/route-corridor/abuja-routes.json — the road geometry the Blue Fleet demo
// cars drive along.
//
//   DOTENV_CONFIG_PATH=.env.demo npx tsx src/scripts/build-abuja-routes.ts
//
...
import { decodePolyline } from '../features/route-corridor/directions.service';
import type { GeoPoint } from '../features/route-corridor/route-corridor.service';
import { ABUJA_LOOP_DEFINITIONS, TRT_OFFICE } from '../features/route-corridor/abuja-routes';
...
(async () => {
  ...
  const file = join(__dirname, '..', 'features', 'route-corridor', 'abuja-routes.json');
